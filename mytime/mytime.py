...
def get_dates(start, end, thisweek, thismonth, thisyear, lastweek, lastmonth, lastyear):
...
@click.option(
    ...
    type=click.Choice(["Area", "Focus", "Proj", "Prof"], case_sensitive=False),
)
@click.option(
    "--tsv",
    default=False,
...
def mytime(
    ...
    start, end = get_dates(
        from_, to, thisweek, thismonth, thisyear, lastweek, lastmonth, lastyear
    )
